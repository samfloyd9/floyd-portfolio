...
function PlayerCard({ playerData, handleDelete }) {
  ...
  // Card title logic
  const cardTitle = playerData.plyrPicture ? `'${playerData.year.slice(-2)} ${playerData.name}` : "";

  // Card layout when a player is selected
  const renderedPlayerCard = (
    <div className="bg-black relative shadow-xl px-1 min-h-44 rounded-lg">
      {/* Position tag in top right corner */}
      <div
        className={`p-0.5 px-1 absolute top-0 right-0 rounded-md ${playerData.teamColor} ${playerData.teamColor2} ${positionStyleLogic(
          playerData
        )} text-md ${playerData.teamColor3} border border-solid flex items-center justify-center`}
      >
        {playerData.position}
      </div>
      ...
      <div className="mt-2">
        <div
          className={`p-1 absolute bottom-0 -right-1 ${playerData.teamColor} text-xs ${playerData.teamColor2} rounded-md ${playerData.teamColor3} border-2 border-solid p-1 text-black z-10 whitespace-nowrap`}
        >
          {cardTitle}
        </div>
      </div>
      ...
      <div>
        <img
          alt={playerData.name}
          src={
            playerData.plyrPicture
          }
          className={`rounded-md shadow-lg my-1.5 max-h-40 ${imageOffset} bg-blend-multiply`}
        />
      </div>
    </div>
  );
  ...
  const playerTemplate = (
    <div className="bg-white shadow-xl w-full px-1.5 min-h-44 rounded-lg flex content-center justify-center text-center items-center">
      {playerData.template}
    </div>
  );
        ...
        <div className="flex flex-col items-center justify-center">
          <div
            className={`${bgImageLogic(
              playerData
            )} shadow-md rounded-xl border border-gray-300 p-1 m-2 flex w-32 justify-center`}
          >
            {renderedPlayerCard}
          </div>
          <div className="flex">
            <button
              ...
              className="bg-red-300 text-xs rounded-lg px-1 -mt-1.5"
            >
              x
            </button>
          </div>
      ...
      {playerData.template && (
        <div>
          <div
            className={`${bgImageLogic(
              playerData
            )} shadow-md rounded-xl border border-gray-300 p-1.5 m-2 flex w-32 justify-center`}
          >
            {playerTemplate}
          </div>
        </div>
